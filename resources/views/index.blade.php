    ...
    <main class="flex justify-around">
        <section class="w-2/6 border-solid border border-gray-400 rounded-md ml-8 my-8">
            ...
            @foreach($folders as $folder)
                <ul>
                    @if ($id==$folder->id)
                        <li class="border-solid border-t border-gray-400 bg-blue-400 text-white"><a class="block px-4 py-2" href="{{route('folders.show', ['id'=>$folder->id])}}">{{$folder->name}}</a></li>
                    @else
                        <li class="border-solid border-t border-gray-400 hover:bg-blue-400 hover:text-white"><a class="block px-4 py-2" href="{{route('folders.show', ['id'=>$folder->id])}}">{{$folder->name}}</a></li>
                    @endif
                </ul>
            @endforeach
        </section>
        <section  class="w-4/6 border-solid border border-gray-400 rounded-md mx-8 my-8">
            ...
            <div class="p-2.5"><p class="p-2 text-center"><a href="" class="block py-2 border-gray-400 border-solid border rounded-md">タスクを追加する</a></p></div>
        </section>
    </main>
</body>
</html>
